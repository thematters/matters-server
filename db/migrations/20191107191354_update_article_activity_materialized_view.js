    ...
    (select article.*,
     ...
     left join
       (select article_id,
        ...
        from comment
        group by article_id) as cc on cc.article_id = article.id
     ...
     left join
       (select reference_id,
        ...
        group by reference_id) as lc1 on lc1.reference_id = article.id
     ...
     left join
       (select reference_id,
        ...
        group by reference_id) as lc2 on lc2.reference_id = article.id

     /* number of days since published */
     left join
       (select id,
               current_date - created_at::date as post_days
        from article) as pd on pd.id = article.id
     ...
     left join
       (select article_id,
        ...
        from comment
        group by article_id) as fc on fc.article_id = article.id
