exports.up = async knex => {
  ...
  from
   (select article.id,
      article.title,
      article.created_at,
      (0.5*coalesce(comment_9_hrs, 0)+2*coalesce(like_18_hrs, 0)) * (2+post_days)/(1+post_days) as base_score,
  ...
  left join
   (select article_id,
          sum((created_at >= now() - interval '9 hours')::int) as comment_9_hrs,
