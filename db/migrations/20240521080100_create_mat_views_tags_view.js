...
WITH temp_tag AS (
  ...
  SELECT t.slug, -- COUNT(*) ::int AS count,
    ...
    COUNT(DISTINCT at.article_id) FILTER(WHERE age(at.created_at) <= '3 months'::interval AND a.state IN ('active') AND a.author_id NOT IN (SELECT user_id FROM user_restriction)) ::int AS num_articles_r3m,
    COUNT(DISTINCT a.author_id) FILTER(WHERE age(at.created_at) <= '3 months'::interval AND a.state IN ('active') AND a.author_id NOT IN (SELECT user_id FROM user_restriction)) ::int AS num_authors_r3m,
    COUNT(DISTINCT at.article_id) FILTER(WHERE age(at.created_at) <= '1 month'::interval AND a.state IN ('active') AND a.author_id NOT IN (SELECT user_id FROM user_restriction)) ::int AS num_articles_r1m,
    COUNT(DISTINCT a.author_id) FILTER(WHERE age(at.created_at) <= '1 month'::interval AND a.state IN ('active') AND a.author_id NOT IN (SELECT user_id FROM user_restriction)) ::int AS num_authors_r1m,
    COUNT(DISTINCT at.article_id) FILTER(WHERE age(at.created_at) <= '2 weeks'::interval AND a.state IN ('active') AND a.author_id NOT IN (SELECT user_id FROM user_restriction)) ::int AS num_articles_r2w,
    COUNT(DISTINCT a.author_id) FILTER(WHERE age(at.created_at) <= '2 weeks'::interval AND a.state IN ('active') AND a.author_id NOT IN (SELECT user_id FROM user_restriction)) ::int AS num_authors_r2w,
    COUNT(DISTINCT at.article_id) FILTER(WHERE age(at.created_at) <= '1 week'::interval AND a.state IN ('active') AND a.author_id NOT IN (SELECT user_id FROM user_restriction)) ::int AS num_articles_r1w,
    COUNT(DISTINCT a.author_id) FILTER(WHERE age(at.created_at) <= '1 week'::interval AND a.state IN ('active') AND a.author_id NOT IN (SELECT user_id FROM user_restriction)) ::int AS num_authors_r1w,
    (MAX(at.created_at) + '1 day'::interval - '1 microsecond'::interval) ::date - MIN(at.created_at) ::date AS span_days,
