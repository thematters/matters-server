...
export const followeeDonatedArticles: RecommendationToFolloweeDonatedArticlesResolver = async (
  { id },
  { input },
  ...
  const data = await Promise.all(
    txs.map(async (tx) => {
      const article = await articleService.linkedDraftLoader.load(tx.targetId)
